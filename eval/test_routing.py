...
# Egyptian REMOVED as a supported dialect (2026-07-09, owner decision): its markers (strong
# and weak alike) were deleted from _detect_dialect entirely — the function is now a plain
# Najdi-marker-membership check, so none of these carry any signal any more.
check("REGR إزيك alone no longer Egyptian",       D("إزيك عامل إيه"), None)
check("REGR مش alone is not Egyptian",            D("مش عارف كيف أروح"), None)
check("REGR weak+strong no longer Egyptian",      D("مش عايز أروح دلوقتي"), None)
check("REGR إيه+ده no longer Egyptian",           D("إيه ده"), None)
...
# Egyptian REMOVED as a supported dialect (2026-07-09, owner decision):
check("Egyptian no longer a request",      R("switch to Egyptian"), None)
check("Masri no longer a request",         R("say it in Masri"), None)
check("بالمصري no longer a request",       R("رد بالمصري"), None)
check("'Egyptian dialect' → unknown note", server._route_turn("Tell me about Egypt in Egyptian dialect", "en")["route"], "unknown_dialect")
check("بالمصري spoken → Fusha default",    server._route_turn("رد بالمصري عن الطقس", "ar")["tts_language"], "standard arabic")
...
check("REGR لا ترد بالنجدي → فصحى",        "Fusha" in (R("لا ترد باللهجة النجدية، رد بالفصحى") or ""), True)
...
_r = server._route_turn("وش أفضل مطعم بالرياض", "ar")
check("route dict: route kind",            _r["route"], "spoken_arabic")
check("route dict: detected dialect",      _r["detected_dialect"], "Najdi")
check("route dict: no requested dialect",  _r["requested_dialect"], None)
_r = server._route_turn("Tell me about Saudi history in Najdi dialect", "en")
check("route dict: explicit request kind", _r["route"], "explicit_arabic")
check("route dict: requested name",        _r["requested_dialect"], "Najdi")
...
check("REGR Egyptian speech no longer detected", route("عايز أعرف إزاي أروح المطار", "ar"), ("saudi", "standard arabic"))
...
check("REGR negated Najdi → Fusha",        route("لا ترد باللهجة النجدية، رد بالفصحى", "ar"), ("saudi", "standard arabic"))
...
_najdi_leaks, _najdi_drift = lint.find_leaks("هذا مكان زين جداً حيث الأجواء لطيفة", "Najdi")
check("جدا is a hard leak now",              "جدا" in _najdi_leaks, True)
check("حيث stays soft MSA drift",            "حيث" in _najdi_drift, True)
...
check("no Egyptian entry left in FORBIDDEN", "Egyptian" in lint.FORBIDDEN, False)
...
check("keeps: unknown-dialect note",         M("أتحدث باللهجات النجدية والفصحى، أرجو أن تكرر اسم اللهجة."), False)
...
check("fixup: egyptian arabic gone (no-op)", F("الوضع كبير جداً هنا", "egyptian arabic"), "الوضع كبير جداً هنا")
...
_fx: list = []
F("الموضوع كبير جداً والتفاصيل جداً مهمة", "najdi arabic", _fx)
check("fixup: labels logged once per chunk", _fx, ["جداً→مرة"])
...
check("linter: مزيان leaks in Najdi",         "مزيان" in leaks("الناس عندها مزيان وحب للضيف", "Najdi"), True)
...
_named = server._route_turn("Tell me about Saudi history in Najdi dialect", "en")
...
check("najdi card: spoken REGISTER note",    "VOICE conversation" in najdi_instr, True)
...
check("no Egyptian card remains",            "Egyptian" in server._DIALECT_CARDS, False)
...
check("unknown note lists no Egyptian",      "Egyptian" in _unk_note, False)
